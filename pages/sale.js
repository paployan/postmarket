...
import { fetchDiscountedProducts } from '../store/actions/products';
...
const OfferOfTheDay = ({ products }) => {
    ...
    return (
        <>
            <Head>
                <title>{t('common:sale')} | Postmarket</title>
            </Head>
            <DefaultLayout>
...
export const getServerSideProps = wrapper.getServerSideProps(
    async ({ store, query, locale }) => {
        await store.dispatch(fetchDiscountedProducts(locale, 12, (query.page || 1)));
    }
);

const mapStateToProps = state => ({
  products: state.products.discounted,
});
